      ...
      <div className="flex flex-col gap-6 w-full">
        ...
        <div className="p-6">
                ...
                {patientData && (
                  <div className="py-[1.5vw] px-[7vw] rounded-2xl shadow-xl bg-white grid grid-rows-3 gap-3">
                    ...
                    <h2 className="text-xl">
                      Your Hospital is: {patientData.HospitalId && patientData.HospitalId.name}
                    </h2>
                    <h2 className="text-xl">
                      Your Doctor is: {patientData.DoctorId && patientData.DoctorId.name}
                    </h2>
                    <div className="w-full flex justify-center">
                      <h1 className="text-3xl font-bold">
                        {patientData.DoctorId && patientData.DoctorId.CurrentNumber}
                      </h1>
                    </div>
          ...
          <div>
            <div>
              <input
                className="h-10 p-2 rounded-xl shadow-lg m-2"
              ...
              />
              <button className="rounded-full bg-blue-500 px-5 py-2 font-semibold">
                Find Hospitals
              </button>
            </div>
          </div>
          <div className="grid grid-cols-3 gap-4">
            {hospitals &&
              hospitals.map((ele) => {
                return <HospitalCard key={ele.id} hospital={ele} />;
              })}
          </div>
        </div>
      </div>
